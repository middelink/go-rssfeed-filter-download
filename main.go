...
import (
	...
	"github.com/middelink/qnap/go-downloadstation"
	"github.com/middelink/qnap/go-filestation"
	"github.com/mmcdole/gofeed"
)

var (
	baseuri = flag.String("baseuri", "", "url for the qnap, e.g. http://192.168.1.5:8080/")
...
func main() {
	...
	qf, err := filestation.New(*baseuri, user, pass)
	if err != nil {
		fmt.Print(err)
		os.Exit(1)
	}
	...
	qd, err := downloadstation.New(*baseuri, user, pass)
	if err != nil {
		fmt.Print(err)
		os.Exit(1)
	}
	defer qd.Close()
	var tasks map[string]downloadstation.TaskState
